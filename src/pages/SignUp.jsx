...
const SignUp = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post('https://ai-prosthetic-limb-control-system-production.up.railway.app/api/v1/register/', {
        username,
        email,
        password,
        role,
  ...
  return (
    <div className="flex-center" style={{ minHeight: '100vh', padding: '20px' }}>
      <div className="glass-panel animate-fade-in" style={{ maxWidth: '450px', width: '100%', padding: '48px' }}>
        ...
        {error && (
          <div style={{ padding: '12px', borderRadius: '8px', background: 'rgba(239, 68, 68, 0.1)', border: '1px solid var(--status-error)', color: 'var(--status-error)', fontSize: '0.85rem', marginBottom: '20px', textAlign: 'center' }}>
            {error}
          </div>
        )}
        <form onSubmit={handleSignUp}>
          <div className="mb-3">
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type="text" className="input-glass" placeholder="Choose a username" style={{ paddingLeft: '48px' }} value={username} onChange={(e) => setUsername(e.target.value)} required />
            </div>
          </div>
          <div className="mb-3">
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type="email" className="input-glass" placeholder="email@example.com" style={{ paddingLeft: '48px' }} value={email} onChange={(e) => setEmail(e.target.value)} required />
            </div>
          </div>
          <div className="mb-3">
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input type="password" className="input-glass" placeholder="••••••••" style={{ paddingLeft: '48px' }} value={password} onChange={(e) => setPassword(e.target.value)} required />
            </div>
          </div>
          <div className="mb-3">
            ...
            <div style={{ position: 'relative' }}>
              <Lock size={18} style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
              <input type="password" className="input-glass" placeholder="••••••••" style={{ paddingLeft: '48px' }} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
            </div>
          </div>
          <div className="mb-4">
            <label className="text-muted" style={{ fontSize: '0.9rem', marginBottom: '12px', display: 'block' }}>Primary Role</label>
            <div className="grid-cols-2" style={{ gap: '12px' }}>
              <button type="button" onClick={() => setRole('patient')} className={role === 'patient' ? 'btn-primary' : 'btn-secondary'} style={{ padding: '10px' }}>Patient</button>
              <button type="button" onClick={() => setRole('healthcare')} className={role === 'healthcare' ? 'btn-primary' : 'btn-secondary'} style={{ padding: '10px' }}>Doctor</button>
            </div>
          </div>
